# (C# 코딩) 버거 주문 키오스크

## 개요
- C# 프로그래밍 학습
  - 1줄 소개: 버거와 추가 옵션을 선택하여 주문 내역과 총 금액을 확인할 수 있는 키오스크 프로그램
- 사용한 플랫폼:
  - C#, .NET Windows Forms, Visual Studio, GitHub
- 사용한 컨트롤:
  - Label, GroupBox, RadioButton, CheckBox, PictureBox, ListBox, Button
- 사용한 기술과 구현한 기능:
  - Visual Studio를 이용하여 UI 디자인
  - RadioButton을 이용한 버거 메뉴 단일 선택
  - CheckBox를 이용한 추가 옵션 복수 선택
  - 주문하기 버튼 클릭 시 주문 내역과 총 금액 출력
  - 초기화 버튼 클릭 시 모든 선택과 출력 내용 초기화
  - 메뉴를 선택하지 않았을 때 Label을 이용한 에러 메시지 출력


 ## 과제1

## 실행 화면
- 코드의 실행 스크린샷과 구현 내용 설명


![실행화면](img/img1.png)



- 구현한 내용 (위 그림 참조)
  - 버거 주문 키오스크 화면 구성
  - 메뉴 선택 영역: 햄버거, 불고기버거, 치킨버거 중 하나 선택 가능
  - 추가 옵션 영역: 감자튀김, 콜라, 치즈 추가, 소스 추가 중 여러 개 선택 가능
  - 주문 내역 영역: 선택한 메뉴와 옵션 표시
  - 총 금액 영역: 선택한 항목들의 가격 합계 표시



# 과제2

## 실행 화면
- 코드의 실행 스크린샷과 구현 내용 설명




![실행화면](img/img2.png)



- 구현한 내용 (위 그림 참조)
  - 주문하기 버튼 기능 구현
  - 선택한 메뉴와 추가 옵션을 ListBox에 출력
  - 선택한 항목의 가격을 계산하여 총 금액 Label에 표시

# 과제3

## 실행 화면
- 코드의 실행 스크린샷과 구현 내용 설명


![실행화면](img/img3.png)



- 구현한 내용 (위 그림 참조)
  - 메뉴를 선택하지 않고 주문하기 버튼을 눌렀을 때 에러 메시지 출력
  - MessageBox 대신 주문하기 버튼 근처 Label에 안내 문구 표시



# 과제4

## 실행 화면
- 코드의 실행 스크린샷과 구현 내용 설명


![실행화면](img/img4.png)



- 구현한 내용 (위 그림 참조)
  - 초기화 버튼 기능 구현
  - 선택한 RadioButton과 CheckBox 해제
  - 주문 내역 삭제
  - 총 금액을 0원으로 초기화
  - 에러 메시지 삭제
 

## 배운 내용
- RadioButton과 CheckBox의 차이를 이해할 수 있었다.
- RadioButton은 하나만 선택하는 메뉴 선택에 적합하고, CheckBox는 여러 개를 선택하는 추가 옵션에 적합하다는 것을 배웠다.
- Checked 속성을 이용하여 사용자의 선택 여부를 확인하고 조건문으로 처리하는 방법을 연습할 수 있었다.
- ListBox와 Label을 이용하여 계산 결과를 화면에 출력하는 방법을 익혔다.
- 에러 메시지를 MessageBox 대신 Label로 출력하면 사용자 화면의 흐름을 덜 방해한다는 점을 알게 되었다.
- Windows Forms에서 여러 컨트롤을 배치하고 이름을 지정하는 과정이 중요하다는 것을 배웠다.